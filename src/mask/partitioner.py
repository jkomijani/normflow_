...
class ChunkCatPartitioner:
    """For chunking the input along the `chunk_axis`."""
...
class AlongAxisEvenOddPartitioner:
    """For even odd slicing the input along the `even_odd_axis`."""

    def __init__(self, even_odd_axis):
        self.axis = even_odd_axis
        self.even_ind = [slice(None)] * (self.axis - 1) + [slice(0, None, 2)]
        self.odd_ind = [slice(None)] * (self.axis - 1) + [slice(1, None, 2)]
